...
library LibERC1155 {
    ...
    */
    ...
    function onERC1155Received(
        ...
        emit TransferSingle(LibMeta.msgSender(), _from, _to, _id, _value);
        uint256 size;
        assembly {
        ...
        if (size > 0) {
            require(
                ERC1155_ACCEPTED == IERC1155TokenReceiver(_to).onERC1155Received(LibMeta.msgSender(), _from, _id, _value, _data),
    ...
    function onERC1155BatchReceived(
        ...
        emit TransferBatch(LibMeta.msgSender(), _from, _to, _ids, _values);
        uint256 size;
        assembly {
            size := extcodesize(_to)
        }
        if (size > 0) {
            require(
                ERC1155_BATCH_ACCEPTED == IERC1155TokenReceiver(_to).onERC1155BatchReceived(LibMeta.msgSender(), _from, _ids, _values, _data),
                "Wearables: Transfer rejected/failed by _to"
            );
        }
    }
