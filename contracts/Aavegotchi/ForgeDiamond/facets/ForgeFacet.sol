...
contract ForgeFacet is Modifiers {
    ...
    function fixInvalidTokenIds(address owner) external onlyDaoOrOwner {
        ForgeTokenFacet tokenFacet = ForgeTokenFacet(address(this));

        uint256 invalidBal1 = tokenFacet.balanceOf(owner, 1000000044);
        uint256 invalidBal2 = tokenFacet.balanceOf(owner, 1000000045);
        uint256 invalidBal3 = tokenFacet.balanceOf(owner, 1000000046);
        uint256 invalidBal4 = tokenFacet.balanceOf(owner, 1000000047);
        uint256 invalidBal5 = tokenFacet.balanceOf(owner, 1000000048);
        uint256 invalidBal6 = tokenFacet.balanceOf(owner, 1000000049);

        if (invalidBal1 > 0) {
            _burnItem(owner, 1000000044, invalidBal1);
            _mintItem(owner, 1000000038, invalidBal1);
        }
        if (invalidBal2 > 0) {
            _burnItem(owner, 1000000045, invalidBal2);
            _mintItem(owner, 1000000039, invalidBal2);
        }
        if (invalidBal3 > 0) {
            _burnItem(owner, 1000000046, invalidBal3);
            _mintItem(owner, 1000000040, invalidBal3);
        }
        if (invalidBal4 > 0) {
            _burnItem(owner, 1000000047, invalidBal4);
            _mintItem(owner, 1000000041, invalidBal4);
        }
        if (invalidBal5 > 0) {
            _burnItem(owner, 1000000048, invalidBal5);
            _mintItem(owner, 1000000042, invalidBal5);
        }
        if (invalidBal6 > 0) {
            _burnItem(owner, 1000000049, invalidBal6);
            _mintItem(owner, 1000000043, invalidBal6);
        }
    }
