...
const itemTypes = [
  {
    ...
    slotPositions: [false, false, false, true, false, false, false, false, false, false, false, false, false, false, false, false],
    category: 0,
...
describe("Re-deploying Contracts, replacing facet selectors, and uploading H1 collateral types", async function() {
  ...
  describe("Haunt", function() {
    ...
    it("Cannot create new haunt until first is finished", async function() {
      ...
      await expect(daoFacet.createHaunt("10000", purchaseNumber, "0x000000")).to.be.revertedWith("AavegotchiFacet: Haunt must be full before creating new");
    });
  });
  ...
  describe("Collaterals and escrow", async function() {
    ...
    it("Can increase stake", async function() {
      ...
      await collateralFacet.increaseStake(testAavegotchiId, amount2Increase.toString());
    ...
    it("Can decrease stake, but not below minimum", async function() {
      ...
      await expect(collateralFacet.decreaseStake(testAavegotchiId, currentStake)).to.be.revertedWith("CollateralFacet: Cannot reduce below minimum stake");
  ...
  describe("DAO Functions", async function() {
    it("Cannot add the same collateral twice", async function() {
      await expect(daoFacet.addCollateralTypes(currentHauntId, getCollaterals("matic", ghstTokenContract.address))).to.be.revertedWith("DAOFacet: Collateral already added");
    });
  });
});
