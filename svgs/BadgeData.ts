export const szn1BadgeSvgs = [
  "163_szn1rnd1top10rarity",
  "164_szn1rnd1top10kinship",
  "165_szn1rnd1top10xp",
  "166_szn1rnd1top100rarity",
  "167_szn1rnd1top100kinship",
  "168_szn1rnd1top100xp",
  "169_szn1rnd2top10rarity",
  "170_szn1rnd2top10kinship",
  "171_szn1rnd2top10xp",
  "172_szn1rnd2top100rarity",
  "173_szn1rnd2top100kinship",
  "174_szn1rnd2top100xp",
  "176_szn1rnd3top10rarity",
  "177_szn1rnd3top10kinship",
  "178_szn1rnd3top10xp",
  "179_szn1rnd3top100rarity",
  "180_szn1rnd3top100kinship",
  "181_szn1rnd3top100xp",
  "182_szn1rnd4top10rarity",
  "183_szn1rnd4top10kinship",
  "184_szn1rnd4top10xp",
  "185_szn1rnd4top100rarity",
  "186_szn1rnd4top100kinship",
  "187_szn1rnd4top100xp",
  "188_szn1finals1stPlaceRarity",
  "189_szn1finals1stPlaceKinship",
  "190_szn1finals1stPlaceXP",
  "191_szn1finals2ndPlaceRarity",
  "192_szn1finals2ndPlaceKinship",
  "193_szn1finals2ndPlaceXP",
  "194_szn1finals3rdPlaceRarity",
  "195_szn1finals3rdPlaceKinship",
  "196_szn1finals3rdPlaceXP",
  "197_szn1plaayer",
];

export const szn1BadgeIds = [
  163, 164, 165, 166, 167, 168, 169, 170, 171, 172, 173, 174, 176, 177, 178,
  179, 180, 181, 182, 183, 184, 185, 186, 187, 188, 189, 190, 191, 192, 193,
  194, 195, 196, 197,
];

//coindesk consensus 2021
export const coindeskConsensus = [198];
export const coindeskConsensusSvg = ["198_consensus"];

//unicly baadge
export const uniclyBaadgeId = [175];
export const uniclyBaadgeSvg = ["175_uniclyBaadge"];

export const szn2BadgeSvgs = [
  "Aavegotchi-RF-SZN2-Trophy-RARITY-1ST",
  "Aavegotchi-RF-SZN2-Trophy-KINSHIP-1ST",
  "Aavegotchi-RF-SZN2-Trophy-XP-1ST",
  "Aavegotchi-RF-SZN2-Trophy-RARITY-2ND",
  "Aavegotchi-RF-SZN2-Trophy-KINSHIP-2ND",
  "Aavegotchi-RF-SZN2-Trophy-XP-2ND",
  "Aavegotchi-RF-SZN2-Trophy-RARITY-3RD",
  "Aavegotchi-RF-SZN2-Trophy-KINSHIP-3RD",
  "Aavegotchi-RF-SZN2-Trophy-XP-3RD",
  "Aavegotchi-RF-SZN2-Baadges-PLAAYER-RAANKED",
  "Aavegotchi-RF-SZN2-Baadges-ROOKIE-OF-THE-YEAR",
  "Aavegotchi-RF-SZN2-Trophy-KINSHIP-ROOKIE-1ST",
  "Aavegotchi-RF-SZN2-Trophy-XP-ROOKIE-1ST",
  "Aavegotchi-RF-SZN2-Trophy-KINSHIP-ROOKIE-2ND",
  "Aavegotchi-RF-SZN2-Trophy-XP-ROOKIE-2ND",
  "Aavegotchi-RF-SZN2-Trophy-KINSHIP-ROOKIE-3RD",
  "Aavegotchi-RF-SZN2-Trophy-XP-ROOKIE-3RD",
  "Aavegotchi-RF-SZN2-Baadges-RARITY-T10",
  "Aavegotchi-RF-SZN2-Baadges-KINSHIP-T10",
  "Aavegotchi-RF-SZN2-Baadges-XP-T10",
  "Aavegotchi-RF-SZN2-Baadges-RARITY-T100",
  "Aavegotchi-RF-SZN2-Baadges-KINSHIP-T100",
  "Aavegotchi-RF-SZN2-Baadges-XP-T100",
  "Aavegotchi-RF-SZN2-Baadges-XP-ROOKIE-T10",
  "Aavegotchi-RF-SZN2-Baadges-KINSHIP-ROOKIE-T10",
  "Aavegotchi-RF-SZN2-Baadges-XP-ROOKIE-T100",
  "Aavegotchi-RF-SZN2-Baadges-KINSHIP-ROOKIE-T100",
];

//unicly baadge
export const aastronautMemberBadgeId = [264];
export const aastronautMemberBadgeSvg = ["264_Aastronaut_member"];

export const szn2BadgeIds = [
  265, 266, 267, 268, 269, 270, 271, 272, 273, 274, 275, 276, 277, 278, 279,
  280, 281, 282, 283, 284, 285, 286, 287, 288, 289, 290, 291,
];

export const szn3BadgeSvgs = [
  "Aavegotchi-RF-SZN3-Trophy-RARITY-CHAMP",
  "Aavegotchi-RF-SZN3-Trophy-KINSHIP-CHAMP",
  "Aavegotchi-RF-SZN3-Trophy-XP-CHAMP",
  "Aavegotchi-RF-SZN3-Trophy-RARITY-2ND",
  "Aavegotchi-RF-SZN3-Trophy-KINSHIP-2ND",
  "Aavegotchi-RF-SZN3-Trophy-XP-2ND",
  "Aavegotchi-RF-SZN3-Trophy-RARITY-3RD",
  "Aavegotchi-RF-SZN3-Trophy-KINSHIP-3RD",
  "Aavegotchi-RF-SZN3-Trophy-XP-3RD",
  "Aavegotchi-RF-SZN3-Baadges-PLAAYER-RAANKED",
  "Aavegotchi-RF-SZN3-Baadges-RARITY-T10",
  "Aavegotchi-RF-SZN3-Baadges-KINSHIP-T10",
  "Aavegotchi-RF-SZN3-Baadges-XP-T10",
  "Aavegotchi-RF-SZN3-Baadges-RARITY-T100",
  "Aavegotchi-RF-SZN3-Baadges-KINSHIP-T100",
  "Aavegotchi-RF-SZN3-Baadges-XP-T100",
];
export const szn3BadgeIds = [
  316, 317, 318, 319, 320, 321, 322, 323, 324, 325, 326, 327, 328, 329, 330,
  331,
];

export const pumpkinBadgeSvgs = ["GotchiSmaash2022-Baadge-Final-Pixelart"];
export const pumpkinBadgeIds = [332];

export const tooorkeyBadgeSvgs = ["Aavegotchi-TOOORKEY-CHAASE-Badge"];
export const tooorkeyBadgeIds = [333];

export const szn4BadgeSvgs = [
  "Aavegotchi-RF-SZN4-TROPHY-CHAMP-RARITY",
  "Aavegotchi-RF-SZN4-TROPHY-CHAMP-KINSHIP",
  "Aavegotchi-RF-SZN4-TROPHY-CHAMP-XP",
  "Aavegotchi-RF-SZN4-TROPHY-2ND-RARITY",
  "Aavegotchi-RF-SZN4-TROPHY-2ND-KINSHIP",
  "Aavegotchi-RF-SZN4-TROPHY-2ND-XP",
  "Aavegotchi-RF-SZN4-TROPHY-3RD-RARITY",
  "Aavegotchi-RF-SZN4-TROPHY-3RD-KINSHIP",
  "Aavegotchi-RF-SZN4-TROPHY-3RD-XP",
  "Aavegotchi-RF-SZN4-Baadge-RAANKED",
  "Aavegotchi-RF-SZN4-Baadge-TOP10-RARITY",
  "Aavegotchi-RF-SZN4-Baadge-TOP10-KINSHIP",
  "Aavegotchi-RF-SZN4-Baadge-TOP10-XP",
  "Aavegotchi-RF-SZN4-Baadge-TOP100-RARITY",
  "Aavegotchi-RF-SZN4-Baadge-TOP100-KINSHIP",
  "Aavegotchi-RF-SZN4-Baadge-TOP100-XP",
];
export const szn4BadgeIds = [
  334, 335, 336, 337, 338, 339, 340, 341, 342, 343, 344, 345, 346, 347, 348,
  349,
];

export const szn5BadgeSvgs = [
  "Aavegotchi-RF-SZN5-Trophy-1ST-PLACE-RARITY", //388
  "Aavegotchi-RF-SZN5-Trophy-1ST-PLACE-KINSHIP", //389
  "Aavegotchi-RF-SZN5-Trophy-1ST-PLACE-XP", //390
  "Aavegotchi-RF-SZN5-Trophy-2ND-PLACE-RARITY", //391
  "Aavegotchi-RF-SZN5-Trophy-2ND-PLACE-KINSHIP", //392
  "Aavegotchi-RF-SZN5-Trophy-2ND-PLACE-XP", //393
  "Aavegotchi-RF-SZN5-Trophy-3RD-PLACE-RARITY", //394
  "Aavegotchi-RF-SZN5-Trophy-3RD-PLACE-KINSHIP", //395
  "Aavegotchi-RF-SZN5-Trophy-3RD-PLACE-XP", //396

  "Aavegotchi-RF-SZN5-Baadge-TOP-RAANKED-PLAAYER", //397
  "Aavegotchi-RF-SZN5-Baadge-TOP-10-RARITY", //398
  "Aavegotchi-RF-SZN5-Baadge-TOP-10-KINSHIP", //399
  "Aavegotchi-RF-SZN5-Baadge-TOP-10-XP", //400
  "Aavegotchi-RF-SZN5-Baadge-TOP-100-RARITY", //401
  "Aavegotchi-RF-SZN5-Baadge-TOP-100-KINSHIP", //402
  "Aavegotchi-RF-SZN5-Baadge-TOP-100-XP", //403
];

export const szn5BadgeIds = [
  388, 389, 390, 391, 392, 393, 394, 395, 396, 397, 398, 399, 400, 401, 402,
  403,
];

export const allBadges: number[] = [
  163, 164, 165, 166, 167, 168, 169, 170, 171, 172, 173, 174, 175, 176, 177,
  178, 179, 180, 181, 182, 183, 184, 185, 186, 187, 188, 189, 190, 191, 192,
  193, 194, 195, 196, 197, 198, 264, 265, 266, 267, 268, 269, 270, 271, 272,
  273, 274, 275, 276, 277, 278, 279, 280, 281, 282, 283, 284, 285, 286, 287,
  288, 289, 290, 291, 316, 317, 318, 319, 320, 321, 322, 323, 324, 325, 326,
  327, 328, 329, 330, 331, 332, 333, 334, 335, 336, 337, 338, 339, 340, 341,
  342, 343, 344, 345, 346, 347, 348, 349, 388, 389, 390, 391, 392, 393, 394,
  395, 396, 397, 398, 399, 400, 401, 402, 403,
];
