const fs = require('fs')

const wearablesRightSvgs = [
  wearable('0_VoidRight'),
  wearable('1_CamoHatRight'),
  wearable('2_CamoPantsRight'), // body but doesn't have sleeves
  wearable('3_MK2GrenadeRight'),
  wearable('4_SnowCamoHatRight'),
  wearable('5_SnowCamoPantsRight'), // body but no sleeves
  wearable('6_M67GrenadeRight'),
  wearable('7_MarineCapRight'),
  bodyWearableRight('8_MarineJacket'), // bodyWearableRight("8_MarineJacket"),
  wearable('9_WalkieTalkie'),
  wearable('10_LinkWhiteHatRight'),
  bodyWearableRight('11_MessDress'), // bodyWearableRight("11_MessDress"),
  wearable('12_LinkBubblyRight'),
  wearable('13_SergeyBeardRight'),
  // wearable('14_SergeyEyesRight'),
  // bodyWearableRight('15_RedPlaid'), // bodyWearableRight("15_RedPlaid"),
  // bodyWearableRight('16_BluePlaid'), //  bodyWearableRight("16_BluePlaid"),
  // wearable('17_LinkCubeRight'),
  // wearable('18_AaveHeroMaskRight'),
  // bodyWearableRight('19_AaveHeroShirtRight'), // bodyWearableRight("19_AaveHeroShirt"),
  // wearable('20_AavePlushRight'),
  // wearable('21_CaptainAaveMaskRight'),
  // bodyWearableRight('22_CaptainAaveSuitRight'), // bodyWearableRight("22_CaptainAaveSuit"),
  // wearable('23_CaptainAaveShieldRight'),
  // wearable('24_ThaaveHelmetRight'),
  // wearable('25_ThaaveSuitRight'), // bodyWearableRight("25_ThaaveSuit"),
  // wearable('26_ThaaveHammerRight'),
  // wearable('27_MarcHairRight'),
  // bodyWearableRight('28_MarcOutfitRight'), // bodyWearableRight("28_MarcOutfit"),
  // wearable('29_REKTSignRight'),
  // wearable('30_JordanHairRight'),
  // bodyWearableRight('31_JordanSuitRight'), // bodyWearableRight("31_JordanSuit"),
  // wearable('32_AaveFlagRight'),
  // wearable('33_StaniHairRight'),
  // wearable('34_StaniVestRight'), // bodyWearableRight("34_StaniVest"),
  // wearable('35_AaveBoatRight'),
  // wearable('36_ETHMaxiGlassesRight'),
  // bodyWearableRight('37_ETHTShirtRight'),
  // wearable('38_32ETHCoinRight'),
  // wearable('39_FoxyMaskRight'),
  // wearable('40_FoxyTailRight'), // body but no sleeves
  // wearable('41_TrezorWalletRight'),
  // wearable('42_NogaraEagleMaskRight'),
  // bodyWearableRight('43_NogaraEagleArmorRight'),
  // wearable('44_DAOEggRight'),
  // wearable('45_ApeMaskRight'),
  // bodyWearableRight('46_HalfRektShirtRight'),
  // wearable('47_WaifuPillowRight'),
  // wearable('48_XibotMohawkRight'),
  // wearable('49_CoderdanShadesRight'),
  // bodyWearableRight('50_GldnXrossRobeRight'),
  // wearable('51_MudgenDiamondRight'),
  // wearable('52_GalaxyBrainRight'),
  // wearable('53_AllSeeingEyesRight'),
  // bodyWearableRight('54_LlamacornShirtRight'),
  // wearable('55_AagentHeadsetRight'),
  // bodyWearableRight('56_AagentShirtRight'),
  // wearable('57_AagentShadesRight'),
  // wearable('58_AagentPistolRight'),
  // wearable('59_AagentFedoraHatRight'),
  // wearable('60_WizardHatRight'),
  // wearable('61_WizardHatLegendaryRight'),
  // wearable('62_WizardHatMythicalRight'),
  // wearable('63_WizardHatGodlikeRight'),
  // wearable('64_WizardStaffRight'),
  // wearable('65_WizardStaffLegendaryRight'),
  // wearable('66_FutureWizardVisorRight'),
  // wearable('67_FarmerStrawHatRight'),
  // wearable('68_FarmerJeansRight'), // Body but no sleeves
  // wearable('69_FarmerPitchforkRight'),
  // wearable('70_FarmerHandsawRight'),
  // wearable('71_SantagotchiHatRight'),
  // wearable('72_JaayHairpieceRight'),
  // wearable('73_JaayGlassesRight'),
  // bodyWearableRight('74_JaayHaoSuitRight'),
  // wearable('75_OKexSignRight'),
  // wearable('76_BigGHSTTokenRight'),
  // wearable('77_BitcoinBeanieRight'),
  // wearable('78_SkaterJeansRight'), // Body but no sleeves
  // wearable('79_SkateboardRight'),
  // wearable('80_SushiHeadbandRight'),
  // wearable('81_SushiRobeRight'), // Body but not sleeves
  // wearable('82_SushiRollRight'),
  // wearable('83_SushiKnifeRight'),
  // wearable('84_GentlemanHatRight'),
  // bodyWearableRight('85_GentlemanSuitRight'),
  // wearable('86_GentlemanMonocleRight'),
  // wearable('87_MinerHelmetRight'),
  // wearable('88_MinerJeansRight'), // Body but no sleeves
  // wearable('89_MinerPickaxeRight'),
  // wearable('90_PajamaHatRight'),
  // bodyWearableRight('91_PajamaPantsRight'),
  // wearable('92_BedtimeMilkRight'),
  // wearable('93_FluffyBlanketRight'),
  // wearable('94_RunnerSweatbandRight'),
  // wearable('95_RunnerShortsRight'), // Body but no sleeves
  // wearable('96_WaterBottleRight'),
  // wearable('97_PillboxHatRight'),
  // wearable('98_LadySkirtRight'), // Body but no sleeves
  // wearable('99_LadyParasolRight'),
  // wearable('100_LadyClutchRight'),
  // wearable('101_WitchHatRight'),
  // bodyWearableRight('102_WitchCapeRight'),
  // wearable('103_WitchWandRight'),
  // wearable('104_PortalMageHelmetRight'),
  // bodyWearableRight('105_PortalMageArmorRight'),
  // wearable('106_PortalMageAxeRight'),
  // wearable('107_PortalMageBlackAxeRight'),
  // wearable('108_RastaDredsRight'),
  // bodyWearableRight('109_RastaShirtRight'),
  // wearable('110_JamaicanFlagRight'),
  // wearable('111_HazmatHoodRight'),
  // bodyWearableRight('112_HazmatSuitRight'),
  // wearable('113_UraniumRodRight'),
  // bodyWearableRight('114_RedHawaiianShirtRight'),
  // bodyWearableRight('115_BlueHawaiianShirtRight'),
  // wearable('116_CoconutRight'),
  // wearable('117_DealWithItShadesRight'),
  // wearable('118_WaterJugRight'),
  // wearable('119_BabyBottleRight'),
  // wearable('120_MartiniRight'),
  // wearable('121_WineBottleRight'),
  // wearable('122_MilkshakeRight'),
  // wearable('123_AppleJuiceRight'),
  // wearable('124_BeerHelmetRight'),
  // wearable('125_TrackSuitRight'),
  // wearable('126_KinshipPotionRight'),
  // wearable('127_GreaterKinshipPotionRight'),
  // wearable('128_XPPotionRight'),
  // wearable('129_GreaterXPPotionRight'),
  // wearable('130_FireballRight'),
  // wearable('131_DragonHornsRight'),
  // wearable('132_DragonWingsRight'),
  // wearable('133_PointyHornsRight'), // Body wearable but not sleeves
  // wearable('134_L2SignRight'),
  // bodyWearableRight('135_PolygonShirtRight'),
  // wearable('136_PolygonCapRight'),
  // wearable('137_VoteSignRight'),
  // bodyWearableRight('138_SnapshotShirtRight'),
  // wearable('139_SnapshotHatRight'),
  // wearable('140_ElfEarsRight'),
  // wearable('141_GemstoneRingRight'),
  // wearable('142_PrincessTiaraRight'),
  // wearable('143_GoldNecklaceRight'),
  // wearable('144_PrincessHairRight'),
  // wearable('145_GodliLocksRight'),
  // wearable('146_ImperialMoustacheRight'),
  // wearable('147_TinyCrownRight'),
  // wearable('148_RoyalScepterRight'),
  // wearable('149_RoyalCrownRight'),
  // bodyWearableRight('150_RoyalRobesRight'),
  // wearable('151_CommonRoflRight'),
  // wearable('152_UncommonRoflRight'),
  // wearable('153_RareRoflRight'),
  // wearable('154_LegendaryRoflRight'),
  // wearable('155_MythicalRoflRight'),
  // wearable('156_GodlikeRoflRight'),
  // wearable('157_LilPumpGoateeRight'),
  // wearable('158_LilPumpDrinkRight'),
  // wearable('159_LilPumpShadesRight'),
  // bodyWearableRight('160_LilPumpThreadsRight'),
  // wearable('161_LilPumpDreadsRight'),
  // bodyWearableRight('162_MiamiShirtRight'),
  // wearable('199_SteampunkGlassesRight'),
  // wearable('200_SteampunkRight'),
  // bodyWearableRight('201_SteampunkGloveRight'),
  // wearable('202_CyberpunkVRRight'),
  // bodyWearableRight('203_CyberpunkRight'),
  // wearable('204_CyberpunkControlRight'),
  // wearable('205_CoffeeCupRight'),
  // wearable('206_AagentRadioRight')
]

const wearablesBackSvgs = [
  wearable('0_VoidBack'),
  wearable('1_CamoHatBack'),
  wearable('2_CamoPantsBack'), // body but doesn't have sleeves
  wearable('3_MK2GrenadeBack'),
  wearable('4_SnowCamoHatBack'),
  wearable('5_SnowCamoPantsBack'), // body but no sleeves
  wearable('6_M67GrenadeBack'),
  wearable('7_MarineCapBack'),
  bodyWearable('8_MarineJacket'), // bodyWearable("8_MarineJacket"),
  wearable('9_WalkieTalkieRightBack'),
  wearable('10_LinkWhiteHatBack'),
  bodyWearable('11_MessDress'), // bodyWearable("11_MessDress"),
  wearable('12_LinkBubblyBack'),
  // wearable('12_LinkBubbly'),
  wearable('13_SergeyBeardBack'),
  // wearable('14_SergeyEyesBack'),
  // bodyWearable('15_RedPlaidBack'), // bodyWearable("15_RedPlaid"),
  // bodyWearable('16_BluePlaidBack'), //  bodyWearable("16_BluePlaid"),
  // wearable('17_LinkCubeBack'),
  // wearable('18_AaveHeroMaskBack'),
  // bodyWearable('19_AaveHeroShirtBack'), // bodyWearable("19_AaveHeroShirt"),
  // wearable('20_AavePlushBack'),
  // wearable('21_CaptainAaveMaskBack'),
  // bodyWearable('22_CaptainAaveSuitBack'), // bodyWearable("22_CaptainAaveSuit"),
  // wearable('23_CaptainAaveShieldBack'),
  // wearable('24_ThaaveHelmetBack'),
  // wearable('25_ThaaveSuitBack'), // bodyWearable("25_ThaaveSuit"),
  // wearable('26_ThaaveHammerBack'),
  // wearable('27_MarcHairBack'),
  // bodyWearable('28_MarcOutfitBack'), // bodyWearable("28_MarcOutfit"),
  // wearable('29_REKTSignBack'),
  // wearable('30_JordanHairBack'),
  // bodyWearable('31_JordanSuitBack'), // bodyWearable("31_JordanSuit"),
  // wearable('32_AaveFlagBack'),
  // wearable('33_StaniHairBack'),
  // wearable('34_StaniVestBack'), // bodyWearable("34_StaniVest"),
  // wearable('35_AaveBoatBack'),
  // wearable('36_ETHMaxiGlassesBack'),
  // bodyWearable('37_ETHTShirtBack'),
  // wearable('38_32ETHCoinBack'),
  // wearable('39_FoxyMaskBack'),
  // wearable('40_FoxyTailBack'), // body but no sleeves
  // wearable('41_TrezorWalletBack'),
  // wearable('42_NogaraEagleMaskBack'),
  // bodyWearable('43_NogaraEagleArmorBack'),
  // wearable('44_DAOEggBack'),
  // wearable('45_ApeMaskBack'),
  // bodyWearable('46_HalfRektShirtBack'),
  // wearable('47_WaifuPillowBack'),
  // wearable('48_XibotMohawkBack'),
  // wearable('49_CoderdanShadesBack'),
  // bodyWearable('50_GldnXrossRobeBack'),
  // wearable('51_MudgenDiamondBack'),
  // wearable('52_GalaxyBrainBack'),
  // wearable('53_AllSeeingEyesBack'),
  // bodyWearable('54_LlamacornShirtBack'),
  // wearable('55_AagentHeadsetBack'),
  // bodyWearable('56_AagentShirtBack'),
  // wearable('57_AagentShadesBack'),
  // wearable('58_AagentPistolBack'),
  // wearable('59_AagentFedoraHatBack'),
  // wearable('60_WizardHatBack'),
  // wearable('61_WizardHatLegendaryBack'),
  // wearable('62_WizardHatMythicalBack'),
  // wearable('63_WizardHatGodlikeBack'),
  // wearable('64_WizardStaffBack'),
  // wearable('65_WizardStaffLegendaryBack'),
  // wearable('66_FutureWizardVisorBack'),
  // wearable('67_FarmerStrawHatBack'),
  // wearable('68_FarmerJeansBack'), // Body but no sleeves
  // wearable('69_FarmerPitchforkBack'),
  // wearable('70_FarmerHandsawBack'),
  // wearable('71_SantagotchiHatBack'),
  // wearable('72_JaayHairpieceBack'),
  // wearable('73_JaayGlassesBack'),
  // bodyWearable('74_JaayHaoSuitBack'),
  // wearable('75_OKexSignBack'),
  // wearable('76_BigGHSTTokenBack'),
  // wearable('77_BitcoinBeanieBack'),
  // wearable('78_SkaterJeansBack'), // Body but no sleeves
  // wearable('79_SkateboardBack'),
  // wearable('80_SushiHeadbandBack'),
  // wearable('81_SushiRobeBack'), // Body but not sleeves
  // wearable('82_SushiRollBack'),
  // wearable('83_SushiKnifeBack'),
  // wearable('84_GentlemanHatBack'),
  // bodyWearable('85_GentlemanSuitBack'),
  // wearable('86_GentlemanMonocleBack'),
  // wearable('87_MinerHelmetBack'),
  // wearable('88_MinerJeansBack'), // Body but no sleeves
  // wearable('89_MinerPickaxeBack'),
  // wearable('90_PajamaHatBack'),
  // bodyWearable('91_PajamaPantsBack'),
  // wearable('92_BedtimeMilkBack'),
  // wearable('93_FluffyBlanketBack'),
  // wearable('94_RunnerSweatbandBack'),
  // wearable('95_RunnerShortsBack'), // Body but no sleeves
  // wearable('96_WaterBottleBack'),
  // wearable('97_PillboxHatBack'),
  // wearable('98_LadySkirtBack'), // Body but no sleeves
  // wearable('99_LadyParasolBack'),
  // wearable('100_LadyClutchBack'),
  // wearable('101_WitchHatBack'),
  // bodyWearable('102_WitchCapeBack'),
  // wearable('103_WitchWandBack'),
  // wearable('104_PortalMageHelmetBack'),
  // bodyWearable('105_PortalMageArmorBack'),
  // wearable('106_PortalMageAxeBack'),
  // wearable('107_PortalMageBlackAxeBack'),
  // wearable('108_RastaDredsBack'),
  // bodyWearable('109_RastaShirtBack'),
  // wearable('110_JamaicanFlagBack'),
  // wearable('111_HazmatHoodBack'),
  // bodyWearable('112_HazmatSuitBack'),
  // wearable('113_UraniumRodBack'),
  // bodyWearable('114_RedHawaiianShirtBack'),
  // bodyWearable('115_BlueHawaiianShirtBack'),
  // wearable('116_CoconutBack'),
  // wearable('117_DealWithItShadesBack'),
  // wearable('118_WaterJugBack'),
  // wearable('119_BabyBottleBack'),
  // wearable('120_MartiniBack'),
  // wearable('121_WineBottleBack'),
  // wearable('122_MilkshakeBack'),
  // wearable('123_AppleJuiceBack'),
  // wearable('124_BeerHelmetBack'),
  // wearable('125_TrackSuitBack'),
  // wearable('126_KinshipPotionBack'),
  // wearable('127_GreaterKinshipPotionBack'),
  // wearable('128_XPPotionBack'),
  // wearable('129_GreaterXPPotionBack'),
  // wearable('130_FireballBack'),
  // wearable('131_DragonHornsBack'),
  // wearable('132_DragonWingsBack'),
  // wearable('133_PointyHornsBack'), // Body wearable but not sleeves
  // wearable('134_L2SignBack'),
  // bodyWearable('135_PolygonShirtBack'),
  // wearable('136_PolygonCapBack'),
  // wearable('137_VoteSignBack'),
  // bodyWearable('138_SnapshotShirtBack'),
  // wearable('139_SnapshotHatBack'),
  // wearable('140_ElfEarsBack'),
  // wearable('141_GemstoneRingBack'),
  // wearable('142_PrincessTiaraBack'),
  // wearable('143_GoldNecklaceBack'),
  // wearable('144_PrincessHairBack'),
  // wearable('145_GodliLocksBack'),
  // wearable('146_ImperialMoustacheBack'),
  // wearable('147_TinyCrownBack'),
  // wearable('148_RoyalScepterBack'),
  // wearable('149_RoyalCrownBack'),
  // bodyWearable('150_RoyalRobesBack'),
  // wearable('151_CommonRoflBack'),
  // wearable('152_UncommonRoflBack'),
  // wearable('153_RareRoflBack'),
  // wearable('154_LegendaryRoflBack'),
  // wearable('155_MythicalRoflBack'),
  // wearable('156_GodlikeRoflBack'),
  // wearable('157_LilPumpGoateeBack'),
  // wearable('158_LilPumpDrinkBack'),
  // wearable('159_LilPumpShadesBack'),
  // bodyWearable('160_LilPumpThreadsBack'),
  // wearable('161_LilPumpDreadsBack'),
  // bodyWearable('162_MiamiShirtBack'),
  // wearable('199_SteampunkGlassesBack'),
  // wearable('200_SteampunkBack'),
  // bodyWearable('201_SteampunkGloveBack'),
  // wearable('202_CyberpunkVRBack'),
  // bodyWearable('203_CyberpunkBack'),
  // wearable('204_CyberpunkControlBack'),
  // wearable('205_CoffeeCupBack'),
  // wearable('206_AagentRadioBack'),
]

const wearablesLeftSvgs = [
  wearable('0_VoidLeft'),
  wearable('1_CamoHatLeft'),
  wearable('2_CamoPantsLeft'), // body but doesn't have sleeves
  wearable('3_MK2GrenadeLeft'),
  wearable('4_SnowCamoHatLeft'),
  wearable('5_SnowCamoPantsLeft'), // body but no sleeves
  wearable('6_M67GrenadeLeft'),
  wearable('7_MarineCapLeft'),
  bodyWearableLeft('8_MarineJacket'), // bodyWearableLeft("8_MarineJacket"),
  wearable('9_WalkieTalkieRightSide'), //is not left for walkie talkie
  wearable('10_LinkWhiteHatLeft'),
  bodyWearableLeft('11_MessDress'), // bodyWearableLeft("11_MessDress"),
  wearable('12_LinkBubblyLeft'),
  wearable('13_SergeyBeardLeft'),
  // wearable('14_SergeyEyesLeft'),
  // bodyWearableLeft('15_RedPlaidLeft'), // bodyWearableLeft("15_RedPlaid"),
  // bodyWearableLeft('16_BluePlaidLeft'), //  bodyWearableLeft("16_BluePlaid"),
  // wearable('17_LinkCubeLeft'),
  // wearable('18_AaveHeroMaskLeft'),
  // bodyWearableLeft('19_AaveHeroShirtLeft'), // bodyWearableLeft("19_AaveHeroShirt"),
  // wearable('20_AavePlushLeft'),
  // wearable('21_CaptainAaveMaskLeft'),
  // bodyWearableLeft('22_CaptainAaveSuitLeft'), // bodyWearableLeft("22_CaptainAaveSuit"),
  // wearable('23_CaptainAaveShieldLeft'),
  // wearable('24_ThaaveHelmetLeft'),
  // wearable('25_ThaaveSuitLeft'), // bodyWearableLeft("25_ThaaveSuit"),
  // wearable('26_ThaaveHammerLeft'),
  // wearable('27_MarcHairLeft'),
  // bodyWearableLeft('28_MarcOutfitLeft'), // bodyWearableLeft("28_MarcOutfit"),
  // wearable('29_REKTSignLeft'),
  // wearable('30_JordanHairLeft'),
  // bodyWearableLeft('31_JordanSuitLeft'), // bodyWearableLeft("31_JordanSuit"),
  // wearable('32_AaveFlagLeft'),
  // wearable('33_StaniHairLeft'),
  // wearable('34_StaniVestLeft'), // bodyWearableLeft("34_StaniVest"),
  // wearable('35_AaveBoatLeft'),
  // wearable('36_ETHMaxiGlassesLeft'),
  // bodyWearableLeft('37_ETHTShirtLeft'),
  // wearable('38_32ETHCoinLeft'),
  // wearable('39_FoxyMaskLeft'),
  // wearable('40_FoxyTailLeft'), // body but no sleeves
  // wearable('41_TrezorWalletLeft'),
  // wearable('42_NogaraEagleMaskLeft'),
  // bodyWearableLeft('43_NogaraEagleArmorLeft'),
  // wearable('44_DAOEggLeft'),
  // wearable('45_ApeMaskLeft'),
  // bodyWearableLeft('46_HalfRektShirtLeft'),
  // wearable('47_WaifuPillowLeft'),
  // wearable('48_XibotMohawkLeft'),
  // wearable('49_CoderdanShadesLeft'),
  // bodyWearableLeft('50_GldnXrossRobeLeft'),
  // wearable('51_MudgenDiamondLeft'),
  // wearable('52_GalaxyBrainLeft'),
  // wearable('53_AllSeeingEyesLeft'),
  // bodyWearableLeft('54_LlamacornShirtLeft'),
  // wearable('55_AagentHeadsetLeft'),
  // bodyWearableLeft('56_AagentShirtLeft'),
  // wearable('57_AagentShadesLeft'),
  // wearable('58_AagentPistolLeft'),
  // wearable('59_AagentFedoraHatLeft'),
  // wearable('60_WizardHatLeft'),
  // wearable('61_WizardHatLegendaryLeft'),
  // wearable('62_WizardHatMythicalLeft'),
  // wearable('63_WizardHatGodlikeLeft'),
  // wearable('64_WizardStaffLeft'),
  // wearable('65_WizardStaffLegendaryLeft'),
  // wearable('66_FutureWizardVisorLeft'),
  // wearable('67_FarmerStrawHatLeft'),
  // wearable('68_FarmerJeansLeft'), // Body but no sleeves
  // wearable('69_FarmerPitchforkLeft'),
  // wearable('70_FarmerHandsawLeft'),
  // wearable('71_SantagotchiHatLeft'),
  // wearable('72_JaayHairpieceLeft'),
  // wearable('73_JaayGlassesLeft'),
  // bodyWearableLeft('74_JaayHaoSuitLeft'),
  // wearable('75_OKexSignLeft'),
  // wearable('76_BigGHSTTokenLeft'),
  // wearable('77_BitcoinBeanieLeft'),
  // wearable('78_SkaterJeansLeft'), // Body but no sleeves
  // wearable('79_SkateboardLeft'),
  // wearable('80_SushiHeadbandLeft'),
  // wearable('81_SushiRobeLeft'), // Body but not sleeves
  // wearable('82_SushiRollLeft'),
  // wearable('83_SushiKnifeLeft'),
  // wearable('84_GentlemanHatLeft'),
  // bodyWearableLeft('85_GentlemanSuitLeft'),
  // wearable('86_GentlemanMonocleLeft'),
  // wearable('87_MinerHelmetLeft'),
  // wearable('88_MinerJeansLeft'), // Body but no sleeves
  // wearable('89_MinerPickaxeLeft'),
  // wearable('90_PajamaHatLeft'),
  // bodyWearableLeft('91_PajamaPantsLeft'),
  // wearable('92_BedtimeMilkLeft'),
  // wearable('93_FluffyBlanketLeft'),
  // wearable('94_RunnerSweatbandLeft'),
  // wearable('95_RunnerShortsLeft'), // Body but no sleeves
  // wearable('96_WaterBottleLeft'),
  // wearable('97_PillboxHatLeft'),
  // wearable('98_LadySkirtLeft'), // Body but no sleeves
  // wearable('99_LadyParasolLeft'),
  // wearable('100_LadyClutchLeft'),
  // wearable('101_WitchHatLeft'),
  // bodyWearableLeft('102_WitchCapeLeft'),
  // wearable('103_WitchWandLeft'),
  // wearable('104_PortalMageHelmetLeft'),
  // bodyWearableLeft('105_PortalMageArmorLeft'),
  // wearable('106_PortalMageAxeLeft'),
  // wearable('107_PortalMageBlackAxeLeft'),
  // wearable('108_RastaDredsLeft'),
  // bodyWearableLeft('109_RastaShirtLeft'),
  // wearable('110_JamaicanFlagLeft'),
  // wearable('111_HazmatHoodLeft'),
  // bodyWearableLeft('112_HazmatSuitLeft'),
  // wearable('113_UraniumRodLeft'),
  // bodyWearableLeft('114_RedHawaiianShirtLeft'),
  // bodyWearableLeft('115_BlueHawaiianShirtLeft'),
  // wearable('116_CoconutLeft'),
  // wearable('117_DealWithItShadesLeft'),
  // wearable('118_WaterJugLeft'),
  // wearable('119_BabyBottleLeft'),
  // wearable('120_MartiniLeft'),
  // wearable('121_WineBottleLeft'),
  // wearable('122_MilkshakeLeft'),
  // wearable('123_AppleJuiceLeft'),
  // wearable('124_BeerHelmetLeft'),
  // wearable('125_TrackSuitLeft'),
  // wearable('126_KinshipPotionLeft'),
  // wearable('127_GreaterKinshipPotionLeft'),
  // wearable('128_XPPotionLeft'),
  // wearable('129_GreaterXPPotionLeft'),
  // wearable('130_FireballLeft'),
  // wearable('131_DragonHornsLeft'),
  // wearable('132_DragonWingsLeft'),
  // wearable('133_PointyHornsLeft'), // Body wearable but not sleeves
  // wearable('134_L2SignLeft'),
  // bodyWearableLeft('135_PolygonShirtLeft'),
  // wearable('136_PolygonCapLeft'),
  // wearable('137_VoteSignLeft'),
  // bodyWearableLeft('138_SnapshotShirtLeft'),
  // wearable('139_SnapshotHatLeft'),
  // wearable('140_ElfEarsLeft'),
  // wearable('141_GemstoneRingLeft'),
  // wearable('142_PrincessTiaraLeft'),
  // wearable('143_GoldNecklaceLeft'),
  // wearable('144_PrincessHairLeft'),
  // wearable('145_GodliLocksLeft'),
  // wearable('146_ImperialMoustacheLeft'),
  // wearable('147_TinyCrownLeft'),
  // wearable('148_RoyalScepterLeft'),
  // wearable('149_RoyalCrownLeft'),
  // bodyWearableLeft('150_RoyalRobesLeft'),
  // wearable('151_CommonRoflLeft'),
  // wearable('152_UncommonRoflLeft'),
  // wearable('153_RareRoflLeft'),
  // wearable('154_LegendaryRoflLeft'),
  // wearable('155_MythicalRoflLeft'),
  // wearable('156_GodlikeRoflLeft'),
  // wearable('157_LilPumpGoateeLeft'),
  // wearable('158_LilPumpDrinkLeft'),
  // wearable('159_LilPumpShadesLeft'),
  // bodyWearableLeft('160_LilPumpThreadsLeft'),
  // wearable('161_LilPumpDreadsLeft'),
  // bodyWearableLeft('162_MiamiShirtLeft'),
  // wearable('199_SteampunkGlassesLeft'),
  // wearable('200_SteampunkLeft'),
  // bodyWearableLeft('201_SteampunkGloveLeft'),
  // wearable('202_CyberpunkVRLeft'),
  // bodyWearableLeft('203_CyberpunkLeft'),
  // wearable('204_CyberpunkControlLeft'),
  // wearable('205_CoffeeCupLeft'),
  // wearable('206_AagentRadioLeft')
]

const wearablesLeftSleeveSvgs = [
  // '0_VoidLeft',
  // '1_CamoHatLeft',
  // '2_CamoPantsLeft', // body but doesn't have sleeves
  // '3_MK2GrenadeLeft',
  // '4_SnowCamoHatLeft',
  // '5_SnowCamoPantsLeft', // body but no sleeves
  // '6_M67GrenadeLeft',
  // '7_MarineCapLeft',
  sleeveWearableLeft('8_MarineJacket'), // sleeveWearableLeft("8_MarineJacket"),
  // '9_WalkieTalkieRightSide', //is not left for walkie talkie
  // '10_LinkWhiteHatLeft',
  sleeveWearableLeft('11_MessDress'), // sleeveWearableLeft("11_MessDress"),
  '12_LinkBubblyLeft',
  '13_SergeyBeardLeft',
  // '14_SergeyEyesLeft',
  // sleeveWearableLeft('15_RedPlaidLeft'), // sleeveWearableLeft("15_RedPlaid"),
  // sleeveWearableLeft('16_BluePlaidLeft'), //  sleeveWearableLeft("16_BluePlaid"),
  // '17_LinkCubeLeft',
  // '18_AaveHeroMaskLeft',
  // sleeveWearableLeft('19_AaveHeroShirtLeft'), // sleeveWearableLeft("19_AaveHeroShirt"),
  // '20_AavePlushLeft',
  // '21_CaptainAaveMaskLeft',
  // sleeveWearableLeft('22_CaptainAaveSuitLeft'), // sleeveWearableLeft("22_CaptainAaveSuit"),
  // '23_CaptainAaveShieldLeft',
  // '24_ThaaveHelmetLeft',
  // '25_ThaaveSuitLeft', // sleeveWearableLeft("25_ThaaveSuit"),
  // '26_ThaaveHammerLeft',
  // '27_MarcHairLeft',
  // sleeveWearableLeft('28_MarcOutfitLeft'), // sleeveWearableLeft("28_MarcOutfit"),
  // '29_REKTSignLeft',
  // '30_JordanHairLeft',
  // sleeveWearableLeft('31_JordanSuitLeft'), // sleeveWearableLeft("31_JordanSuit"),
  // '32_AaveFlagLeft',
  // '33_StaniHairLeft',
  // '34_StaniVestLeft', // sleeveWearableLeft("34_StaniVest"),
  // '35_AaveBoatLeft',
  // '36_ETHMaxiGlassesLeft',
  // sleeveWearableLeft('37_ETHTShirtLeft'),
  // '38_32ETHCoinLeft',
  // '39_FoxyMaskLeft',
  // '40_FoxyTailLeft', // body but no sleeves
  // '41_TrezorWalletLeft',
  // '42_NogaraEagleMaskLeft',
  // sleeveWearableLeft('43_NogaraEagleArmorLeft'),
  // '44_DAOEggLeft',
  // '45_ApeMaskLeft',
  // sleeveWearableLeft('46_HalfRektShirtLeft'),
  // '47_WaifuPillowLeft',
  // '48_XibotMohawkLeft',
  // '49_CoderdanShadesLeft',
  // sleeveWearableLeft('50_GldnXrossRobeLeft'),
  // '51_MudgenDiamondLeft',
  // '52_GalaxyBrainLeft',
  // '53_AllSeeingEyesLeft',
  // sleeveWearableLeft('54_LlamacornShirtLeft'),
  // '55_AagentHeadsetLeft',
  // sleeveWearableLeft('56_AagentShirtLeft'),
  // '57_AagentShadesLeft',
  // '58_AagentPistolLeft',
  // '59_AagentFedoraHatLeft',
  // '60_WizardHatLeft',
  // '61_WizardHatLegendaryLeft',
  // '62_WizardHatMythicalLeft',
  // '63_WizardHatGodlikeLeft',
  // '64_WizardStaffLeft',
  // '65_WizardStaffLegendaryLeft',
  // '66_FutureWizardVisorLeft',
  // '67_FarmerStrawHatLeft',
  // '68_FarmerJeansLeft', // Body but no sleeves
  // '69_FarmerPitchforkLeft',
  // '70_FarmerHandsawLeft',
  // '71_SantagotchiHatLeft',
  // '72_JaayHairpieceLeft',
  // '73_JaayGlassesLeft',
  // sleeveWearableLeft('74_JaayHaoSuitLeft'),
  // '75_OKexSignLeft',
  // '76_BigGHSTTokenLeft',
  // '77_BitcoinBeanieLeft',
  // '78_SkaterJeansLeft', // Body but no sleeves
  // '79_SkateboardLeft',
  // '80_SushiHeadbandLeft',
  // '81_SushiRobeLeft', // Body but not sleeves
  // '82_SushiRollLeft',
  // '83_SushiKnifeLeft',
  // '84_GentlemanHatLeft',
  // sleeveWearableLeft('85_GentlemanSuitLeft'),
  // '86_GentlemanMonocleLeft',
  // '87_MinerHelmetLeft',
  // '88_MinerJeansLeft', // Body but no sleeves
  // '89_MinerPickaxeLeft',
  // '90_PajamaHatLeft',
  // sleeveWearableLeft('91_PajamaPantsLeft'),
  // '92_BedtimeMilkLeft',
  // '93_FluffyBlanketLeft',
  // '94_RunnerSweatbandLeft',
  // '95_RunnerShortsLeft', // Body but no sleeves
  // '96_WaterBottleLeft',
  // '97_PillboxHatLeft',
  // '98_LadySkirtLeft', // Body but no sleeves
  // '99_LadyParasolLeft',
  // '100_LadyClutchLeft',
  // '101_WitchHatLeft',
  // sleeveWearableLeft('102_WitchCapeLeft'),
  // '103_WitchWandLeft',
  // '104_PortalMageHelmetLeft',
  // sleeveWearableLeft('105_PortalMageArmorLeft'),
  // '106_PortalMageAxeLeft',
  // '107_PortalMageBlackAxeLeft',
  // '108_RastaDredsLeft',
  // sleeveWearableLeft('109_RastaShirtLeft'),
  // '110_JamaicanFlagLeft',
  // '111_HazmatHoodLeft',
  // sleeveWearableLeft('112_HazmatSuitLeft'),
  // '113_UraniumRodLeft',
  // sleeveWearableLeft('114_RedHawaiianShirtLeft'),
  // sleeveWearableLeft('115_BlueHawaiianShirtLeft'),
  // '116_CoconutLeft',
  // '117_DealWithItShadesLeft',
  // '118_WaterJugLeft',
  // '119_BabyBottleLeft',
  // '120_MartiniLeft',
  // '121_WineBottleLeft',
  // '122_MilkshakeLeft',
  // '123_AppleJuiceLeft',
  // '124_BeerHelmetLeft',
  // '125_TrackSuitLeft',
  // '126_KinshipPotionLeft',
  // '127_GreaterKinshipPotionLeft',
  // '128_XPPotionLeft',
  // '129_GreaterXPPotionLeft',
  // '130_FireballLeft',
  // '131_DragonHornsLeft',
  // '132_DragonWingsLeft',
  // '133_PointyHornsLeft', // Body wearable but not sleeves
  // '134_L2SignLeft',
  // sleeveWearableLeft('135_PolygonShirtLeft'),
  // '136_PolygonCapLeft',
  // '137_VoteSignLeft',
  // sleeveWearableLeft('138_SnapshotShirtLeft'),
  // '139_SnapshotHatLeft',
  // '140_ElfEarsLeft',
  // '141_GemstoneRingLeft',
  // '142_PrincessTiaraLeft',
  // '143_GoldNecklaceLeft',
  // '144_PrincessHairLeft',
  // '145_GodliLocksLeft',
  // '146_ImperialMoustacheLeft',
  // '147_TinyCrownLeft',
  // '148_RoyalScepterLeft',
  // '149_RoyalCrownLeft',
  // sleeveWearableLeft('150_RoyalRobesLeft'),
  // '151_CommonRoflLeft',
  // '152_UncommonRoflLeft',
  // '153_RareRoflLeft',
  // '154_LegendaryRoflLeft',
  // '155_MythicalRoflLeft',
  // '156_GodlikeRoflLeft',
  // '157_LilPumpGoateeLeft',
  // '158_LilPumpDrinkLeft',
  // '159_LilPumpShadesLeft',
  // sleeveWearableLeft('160_LilPumpThreadsLeft'),
  // '161_LilPumpDreadsLeft',
  // sleeveWearableLeft('162_MiamiShirtLeft'),
  // '199_SteampunkGlassesLeft',
  // '200_SteampunkLeft',
  // sleeveWearableLeft('201_SteampunkGloveLeft'),
  // '202_CyberpunkVRLeft',
  // sleeveWearableLeft('203_CyberpunkLeft'),
  // '204_CyberpunkControlLeft',
  // '205_CoffeeCupLeft',
  // '206_AagentRadioLeft'
]

const wearablesRightSleeveSvgs = [
  // '0_VoidRight',
  // '1_CamoHatRight',
  // '2_CamoPantsRight', // body but doesn't have sleeves
  // '3_MK2GrenadeRight',
  // '4_SnowCamoHatRight',
  // '5_SnowCamoPantsRight', // body but no sleeves
  // '6_M67GrenadeRight',
  // '7_MarineCapRight',
  sleeveWearableRight('8_MarineJacket'), // sleeveWearableRight("8_MarineJacket"),
  // '9_WalkieTalkie',
  // '10_LinkWhiteHatRight',
  sleeveWearableRight('11_MessDress'), // sleeveWearableRight("11_MessDress"),
  '12_LinkBubblyRight',
  '13_SergeyBeardRight',
  // '14_SergeyEyesRight',
  // sleeveWearableRight('15_RedPlaid'), // sleeveWearableRight("15_RedPlaid"),
  // sleeveWearableRight('16_BluePlaid'), //  sleeveWearableRight("16_BluePlaid"),
  // '17_LinkCubeRight',
  // '18_AaveHeroMaskRight',
  // sleeveWearableRight('19_AaveHeroShirtRight'), // sleeveWearableRight("19_AaveHeroShirt"),
  // '20_AavePlushRight',
  // '21_CaptainAaveMaskRight',
  // sleeveWearableRight('22_CaptainAaveSuitRight'), // sleeveWearableRight("22_CaptainAaveSuit"),
  // '23_CaptainAaveShieldRight',
  // '24_ThaaveHelmetRight',
  // '25_ThaaveSuitRight', // sleeveWearableRight("25_ThaaveSuit"),
  // '26_ThaaveHammerRight',
  // '27_MarcHairRight',
  // sleeveWearableRight('28_MarcOutfitRight'), // sleeveWearableRight("28_MarcOutfit"),
  // '29_REKTSignRight',
  // '30_JordanHairRight',
  // sleeveWearableRight('31_JordanSuitRight'), // sleeveWearableRight("31_JordanSuit"),
  // '32_AaveFlagRight',
  // '33_StaniHairRight',
  // '34_StaniVestRight', // sleeveWearableRight("34_StaniVest"),
  // '35_AaveBoatRight',
  // '36_ETHMaxiGlassesRight',
  // sleeveWearableRight('37_ETHTShirtRight'),
  // '38_32ETHCoinRight',
  // '39_FoxyMaskRight',
  // '40_FoxyTailRight', // body but no sleeves
  // '41_TrezorWalletRight',
  // '42_NogaraEagleMaskRight',
  // sleeveWearableRight('43_NogaraEagleArmorRight'),
  // '44_DAOEggRight',
  // '45_ApeMaskRight',
  // sleeveWearableRight('46_HalfRektShirtRight'),
  // '47_WaifuPillowRight',
  // '48_XibotMohawkRight',
  // '49_CoderdanShadesRight',
  // sleeveWearableRight('50_GldnXrossRobeRight'),
  // '51_MudgenDiamondRight',
  // '52_GalaxyBrainRight',
  // '53_AllSeeingEyesRight',
  // sleeveWearableRight('54_LlamacornShirtRight'),
  // '55_AagentHeadsetRight',
  // sleeveWearableRight('56_AagentShirtRight'),
  // '57_AagentShadesRight',
  // '58_AagentPistolRight',
  // '59_AagentFedoraHatRight',
  // '60_WizardHatRight',
  // '61_WizardHatLegendaryRight',
  // '62_WizardHatMythicalRight',
  // '63_WizardHatGodlikeRight',
  // '64_WizardStaffRight',
  // '65_WizardStaffLegendaryRight',
  // '66_FutureWizardVisorRight',
  // '67_FarmerStrawHatRight',
  // '68_FarmerJeansRight', // Body but no sleeves
  // '69_FarmerPitchforkRight',
  // '70_FarmerHandsawRight',
  // '71_SantagotchiHatRight',
  // '72_JaayHairpieceRight',
  // '73_JaayGlassesRight',
  // sleeveWearableRight('74_JaayHaoSuitRight'),
  // '75_OKexSignRight',
  // '76_BigGHSTTokenRight',
  // '77_BitcoinBeanieRight',
  // '78_SkaterJeansRight', // Body but no sleeves
  // '79_SkateboardRight',
  // '80_SushiHeadbandRight',
  // '81_SushiRobeRight', // Body but not sleeves
  // '82_SushiRollRight',
  // '83_SushiKnifeRight',
  // '84_GentlemanHatRight',
  // sleeveWearableRight('85_GentlemanSuitRight'),
  // '86_GentlemanMonocleRight',
  // '87_MinerHelmetRight',
  // '88_MinerJeansRight', // Body but no sleeves
  // '89_MinerPickaxeRight',
  // '90_PajamaHatRight',
  // sleeveWearableRight('91_PajamaPantsRight'),
  // '92_BedtimeMilkRight',
  // '93_FluffyBlanketRight',
  // '94_RunnerSweatbandRight',
  // '95_RunnerShortsRight', // Body but no sleeves
  // '96_WaterBottleRight',
  // '97_PillboxHatRight',
  // '98_LadySkirtRight', // Body but no sleeves
  // '99_LadyParasolRight',
  // '100_LadyClutchRight',
  // '101_WitchHatRight',
  // sleeveWearableRight('102_WitchCapeRight'),
  // '103_WitchWandRight',
  // '104_PortalMageHelmetRight',
  // sleeveWearableRight('105_PortalMageArmorRight'),
  // '106_PortalMageAxeRight',
  // '107_PortalMageBlackAxeRight',
  // '108_RastaDredsRight',
  // sleeveWearableRight('109_RastaShirtRight'),
  // '110_JamaicanFlagRight',
  // '111_HazmatHoodRight',
  // sleeveWearableRight('112_HazmatSuitRight'),
  // '113_UraniumRodRight',
  // sleeveWearableRight('114_RedHawaiianShirtRight'),
  // sleeveWearableRight('115_BlueHawaiianShirtRight'),
  // '116_CoconutRight',
  // '117_DealWithItShadesRight',
  // '118_WaterJugRight',
  // '119_BabyBottleRight',
  // '120_MartiniRight',
  // '121_WineBottleRight',
  // '122_MilkshakeRight',
  // '123_AppleJuiceRight',
  // '124_BeerHelmetRight',
  // '125_TrackSuitRight',
  // '126_KinshipPotionRight',
  // '127_GreaterKinshipPotionRight',
  // '128_XPPotionRight',
  // '129_GreaterXPPotionRight',
  // '130_FireballRight',
  // '131_DragonHornsRight',
  // '132_DragonWingsRight',
  // '133_PointyHornsRight', // Body wearable but not sleeves
  // '134_L2SignRight',
  // sleeveWearableRight('135_PolygonShirtRight'),
  // '136_PolygonCapRight',
  // '137_VoteSignRight',
  // sleeveWearableRight('138_SnapshotShirtRight'),
  // '139_SnapshotHatRight',
  // '140_ElfEarsRight',
  // '141_GemstoneRingRight',
  // '142_PrincessTiaraRight',
  // '143_GoldNecklaceRight',
  // '144_PrincessHairRight',
  // '145_GodliLocksRight',
  // '146_ImperialMoustacheRight',
  // '147_TinyCrownRight',
  // '148_RoyalScepterRight',
  // '149_RoyalCrownRight',
  // sleeveWearableRight('150_RoyalRobesRight'),
  // '151_CommonRoflRight',
  // '152_UncommonRoflRight',
  // '153_RareRoflRight',
  // '154_LegendaryRoflRight',
  // '155_MythicalRoflRight',
  // '156_GodlikeRoflRight',
  // '157_LilPumpGoateeRight',
  // '158_LilPumpDrinkRight',
  // '159_LilPumpShadesRight',
  // sleeveWearableRight('160_LilPumpThreadsRight'),
  // '161_LilPumpDreadsRight',
  // sleeveWearableRight('162_MiamiShirtRight'),
  // '199_SteampunkGlassesRight',
  // '200_SteampunkRight',
  // sleeveWearableRight('201_SteampunkGloveRight'),
  // '202_CyberpunkVRRight',
  // sleeveWearableRight('203_CyberpunkRight'),
  // '204_CyberpunkControlRight',
  // '205_CoffeeCupRight',
  // '206_AagentRadioRight'
]

const wearablesBackSleeveSvgs = [
  // '0_VoidRight',
  // '1_CamoHatRight',
  // '2_CamoPantsRight', // body but doesn't have sleeves
  // '3_MK2GrenadeRight',
  // '4_SnowCamoHatRight',
  // '5_SnowCamoPantsRight', // body but no sleeves
  // '6_M67GrenadeRight',
  // '7_MarineCapRight',
  sleeveWearableRight('8_MarineJacket'), // sleeveWearableRight("8_MarineJacket"),
  // '9_WalkieTalkie',
  // '10_LinkWhiteHatRight',
  sleeveWearableRight('11_MessDress'), // sleeveWearableRight("11_MessDress"),
  '12_LinkBubblyRight',
  '13_SergeyBeardRight',
  // '14_SergeyEyesRight',
  // sleeveWearableRight('15_RedPlaid'), // sleeveWearableRight("15_RedPlaid"),
  // sleeveWearableRight('16_BluePlaid'), //  sleeveWearableRight("16_BluePlaid"),
  // '17_LinkCubeRight',
  // '18_AaveHeroMaskRight',
  // sleeveWearableRight('19_AaveHeroShirtRight'), // sleeveWearableRight("19_AaveHeroShirt"),
  // '20_AavePlushRight',
  // '21_CaptainAaveMaskRight',
  // sleeveWearableRight('22_CaptainAaveSuitRight'), // sleeveWearableRight("22_CaptainAaveSuit"),
  // '23_CaptainAaveShieldRight',
  // '24_ThaaveHelmetRight',
  // '25_ThaaveSuitRight', // sleeveWearableRight("25_ThaaveSuit"),
  // '26_ThaaveHammerRight',
  // '27_MarcHairRight',
  // sleeveWearableRight('28_MarcOutfitRight'), // sleeveWearableRight("28_MarcOutfit"),
  // '29_REKTSignRight',
  // '30_JordanHairRight',
  // sleeveWearableRight('31_JordanSuitRight'), // sleeveWearableRight("31_JordanSuit"),
  // '32_AaveFlagRight',
  // '33_StaniHairRight',
  // '34_StaniVestRight', // sleeveWearableRight("34_StaniVest"),
  // '35_AaveBoatRight',
  // '36_ETHMaxiGlassesRight',
  // sleeveWearableRight('37_ETHTShirtRight'),
  // '38_32ETHCoinRight',
  // '39_FoxyMaskRight',
  // '40_FoxyTailRight', // body but no sleeves
  // '41_TrezorWalletRight',
  // '42_NogaraEagleMaskRight',
  // sleeveWearableRight('43_NogaraEagleArmorRight'),
  // '44_DAOEggRight',
  // '45_ApeMaskRight',
  // sleeveWearableRight('46_HalfRektShirtRight'),
  // '47_WaifuPillowRight',
  // '48_XibotMohawkRight',
  // '49_CoderdanShadesRight',
  // sleeveWearableRight('50_GldnXrossRobeRight'),
  // '51_MudgenDiamondRight',
  // '52_GalaxyBrainRight',
  // '53_AllSeeingEyesRight',
  // sleeveWearableRight('54_LlamacornShirtRight'),
  // '55_AagentHeadsetRight',
  // sleeveWearableRight('56_AagentShirtRight'),
  // '57_AagentShadesRight',
  // '58_AagentPistolRight',
  // '59_AagentFedoraHatRight',
  // '60_WizardHatRight',
  // '61_WizardHatLegendaryRight',
  // '62_WizardHatMythicalRight',
  // '63_WizardHatGodlikeRight',
  // '64_WizardStaffRight',
  // '65_WizardStaffLegendaryRight',
  // '66_FutureWizardVisorRight',
  // '67_FarmerStrawHatRight',
  // '68_FarmerJeansRight', // Body but no sleeves
  // '69_FarmerPitchforkRight',
  // '70_FarmerHandsawRight',
  // '71_SantagotchiHatRight',
  // '72_JaayHairpieceRight',
  // '73_JaayGlassesRight',
  // sleeveWearableRight('74_JaayHaoSuitRight'),
  // '75_OKexSignRight',
  // '76_BigGHSTTokenRight',
  // '77_BitcoinBeanieRight',
  // '78_SkaterJeansRight', // Body but no sleeves
  // '79_SkateboardRight',
  // '80_SushiHeadbandRight',
  // '81_SushiRobeRight', // Body but not sleeves
  // '82_SushiRollRight',
  // '83_SushiKnifeRight',
  // '84_GentlemanHatRight',
  // sleeveWearableRight('85_GentlemanSuitRight'),
  // '86_GentlemanMonocleRight',
  // '87_MinerHelmetRight',
  // '88_MinerJeansRight', // Body but no sleeves
  // '89_MinerPickaxeRight',
  // '90_PajamaHatRight',
  // sleeveWearableRight('91_PajamaPantsRight'),
  // '92_BedtimeMilkRight',
  // '93_FluffyBlanketRight',
  // '94_RunnerSweatbandRight',
  // '95_RunnerShortsRight', // Body but no sleeves
  // '96_WaterBottleRight',
  // '97_PillboxHatRight',
  // '98_LadySkirtRight', // Body but no sleeves
  // '99_LadyParasolRight',
  // '100_LadyClutchRight',
  // '101_WitchHatRight',
  // sleeveWearableRight('102_WitchCapeRight'),
  // '103_WitchWandRight',
  // '104_PortalMageHelmetRight',
  // sleeveWearableRight('105_PortalMageArmorRight'),
  // '106_PortalMageAxeRight',
  // '107_PortalMageBlackAxeRight',
  // '108_RastaDredsRight',
  // sleeveWearableRight('109_RastaShirtRight'),
  // '110_JamaicanFlagRight',
  // '111_HazmatHoodRight',
  // sleeveWearableRight('112_HazmatSuitRight'),
  // '113_UraniumRodRight',
  // sleeveWearableRight('114_RedHawaiianShirtRight'),
  // sleeveWearableRight('115_BlueHawaiianShirtRight'),
  // '116_CoconutRight',
  // '117_DealWithItShadesRight',
  // '118_WaterJugRight',
  // '119_BabyBottleRight',
  // '120_MartiniRight',
  // '121_WineBottleRight',
  // '122_MilkshakeRight',
  // '123_AppleJuiceRight',
  // '124_BeerHelmetRight',
  // '125_TrackSuitRight',
  // '126_KinshipPotionRight',
  // '127_GreaterKinshipPotionRight',
  // '128_XPPotionRight',
  // '129_GreaterXPPotionRight',
  // '130_FireballRight',
  // '131_DragonHornsRight',
  // '132_DragonWingsRight',
  // '133_PointyHornsRight', // Body wearable but not sleeves
  // '134_L2SignRight',
  // sleeveWearableRight('135_PolygonShirtRight'),
  // '136_PolygonCapRight',
  // '137_VoteSignRight',
  // sleeveWearableRight('138_SnapshotShirtRight'),
  // '139_SnapshotHatRight',
  // '140_ElfEarsRight',
  // '141_GemstoneRingRight',
  // '142_PrincessTiaraRight',
  // '143_GoldNecklaceRight',
  // '144_PrincessHairRight',
  // '145_GodliLocksRight',
  // '146_ImperialMoustacheRight',
  // '147_TinyCrownRight',
  // '148_RoyalScepterRight',
  // '149_RoyalCrownRight',
  // sleeveWearableRight('150_RoyalRobesRight'),
  // '151_CommonRoflRight',
  // '152_UncommonRoflRight',
  // '153_RareRoflRight',
  // '154_LegendaryRoflRight',
  // '155_MythicalRoflRight',
  // '156_GodlikeRoflRight',
  // '157_LilPumpGoateeRight',
  // '158_LilPumpDrinkRight',
  // '159_LilPumpShadesRight',
  // sleeveWearableRight('160_LilPumpThreadsRight'),
  // '161_LilPumpDreadsRight',
  // sleeveWearableRight('162_MiamiShirtRight'),
  // '199_SteampunkGlassesRight',
  // '200_SteampunkRight',
  // sleeveWearableRight('201_SteampunkGloveRight'),
  // '202_CyberpunkVRRight',
  // sleeveWearableRight('203_CyberpunkRight'),
  // '204_CyberpunkControlRight',
  // '205_CoffeeCupRight',
  // '206_AagentRadioRight'
]

function stripSvg (svg) {
  // removes svg tag
  if (svg.includes('viewBox')) {
    svg = svg.slice(svg.indexOf('>') + 1)
    svg = svg.replace('</svg>', '')
  }
  return svg
}

function readSvg (name) {
  return stripSvg(fs.readFileSync(`./svgs/svgItems/${name}.svg`, 'utf8'))
}

function wearable (name) {
  const svg = readSvg(name)
  // svg = `<g>${svg}</g>`
  return svg
}

function bodyWearable (name) {
  let svg = readSvg(name)
  const back = readSvg(`${name}Back`)
  const backLeftSleevesUp = '<g class="gotchi-sleeves gotchi-sleeves-left gotchi-sleeves-up">' + readSvg(`${name}BackLeftUp`) + '</g>'
  const backLeft = readSvg(`${name}BackLeft`)
  // const sideLeft = readSvg(`${name}SideLeft`)
  // const sideLeftUp = readSvg(`${name}SideLeftUp`)
  // const sideLeftDown = readSvg(`${name}SideLeftDown`)

  const backRightSleevesUp = '<g class="gotchi-sleeves gotchi-sleeves-right gotchi-sleeves-up">' + readSvg(`${name}BackRightUp`) + '</g>'
  const backRight = readSvg(`${name}BackRight`)
  // const sideRight = readSvg(`${name}SideRight`)
  // const sideRightUp = readSvg(`${name}SideRightUp`)
  // const sideRightDown = readSvg(`${name}SideRightDown`)

  svg = '<g>' + back + backLeftSleevesUp + backLeft + backRightSleevesUp + backRight + '</g>'
  return svg
}

function bodyWearableLeft (name) {
  let svg;
  const left = readSvg(`${name}SideLeft`)
  svg = '<g>' + left + '</g>'
  return svg
}

function bodyWearableRight (name) {
  let svg;
  const right = readSvg(`${name}SideRight`)
  svg = '<g>' + right + '</g>'
  return svg
}

function sleeveWearableLeft (name) {
  let svg;
  const leftSleevesUp = '<g class="gotchi-sleeves gotchi-sleeves-left gotchi-sleeves-up">' + readSvg(`${name}SideLeftUp`) + '</g>'
  const leftSleevesDown = '<g class="gotchi-sleeves gotchi-sleeves-left gotchi-sleeves-down">' + readSvg(`${name}SideLeftDown`) + '</g>'
  svg = '<g>' + leftSleevesUp + leftSleevesDown + '</g>'
  return svg
}

function sleeveWearableRight (name) {
  let svg;
  const rightSleevesUp = '<g class="gotchi-sleeves gotchi-sleeves-right gotchi-sleeves-up">' + readSvg(`${name}SideRightUp`) + '</g>'
  const rightSleevesDown = '<g class="gotchi-sleeves gotchi-sleeves-right gotchi-sleeves-down">' + readSvg(`${name}SideRightDown`) + '</g>'
  svg = '<g>' + rightSleevesUp + rightSleevesDown + '</g>'
  return svg
}

exports.wearablesLeftSvgs = wearablesLeftSvgs
exports.wearablesRightSvgs = wearablesRightSvgs
exports.wearablesBackSvgs = wearablesBackSvgs
exports.wearablesLeftSleeveSvgs = wearablesLeftSleeveSvgs
exports.wearablesRightSleeveSvgs = wearablesRightSleeveSvgs
exports.wearablesBackSleeveSvgs = wearablesBackSleeveSvgs
