...
async function main() {
    ...
    const GHSTDiamondFactory = await ethers.getContractFactory('GHSTFacet')
    const ghstLoupeFacet = await GHSTDiamondFactory.deploy()
    await ghstLoupeFacet.deployed()

    console.log('ghst address:', ghstLoupeFacet.address)
    ...
    const aavegotchiDiamond = await diamond.deploy({
        ...
        owner: account,
        otherArgs: [/*ghstContractAddress*/ ghstLoupeFacet.address, getCollaterals(bre.network.name)]
    })
