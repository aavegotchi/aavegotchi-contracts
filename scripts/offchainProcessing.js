...
function modifyWithAavegotchiSets (setData, equipped, traits, rarityScore) {
  ...
  setData = setData.reduce((acc, value) => {
    const setWearableIds = value.wearableIds.reduce(getEquipmentIds, [])
    if (setWearableIds.every(wearableId => equipped.includes(wearableId))) {
      rarityScore += Number(value.traitsBonuses[0])
      for (let i = 1; i < 5; i++) {
        const traitBonus = Number(value.traitsBonuses[i])
  ...
  return [setData, traits, rarityScore]
}
