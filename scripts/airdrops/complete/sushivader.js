...
const { sushivader:game } = require('../../../data/airdrops/sushivader.tsx')
...
let results = Object.keys(game).map((key) => {
    return {
      tokenId: key,
  ...
  results = results.sort(orderByScore);

  let game15 = results.slice(0,51)
  console.log('game15:',game15.length)
  let game10 = results.slice(51,501)

  console.log('game 10:',game10.length)

  let game5 = results.slice(501,results.length)

  game5 = game5.filter((obj) => {
    return obj.score >= 5000
  })
