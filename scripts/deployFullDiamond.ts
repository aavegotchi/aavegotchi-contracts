...
async function main() {
  ...
  const ghstDiamondAddress = "0xF679b8D109b2d23931237Ce948a7D784727c0897";
  ...
  step = 2;
  sliceStep = wearableSetArrays.length / step;
  for (let i = 0; i < step; i++) {
    tx = await daoFacet.addWearableSets(
      wearableSetArrays.slice(sliceStep * i, sliceStep * (i + 1)),
      { gasLimit: gasLimit }
    );
    receipt = await tx.wait();
    if (!receipt.status) {
