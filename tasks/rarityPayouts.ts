...
task("rarityPayout").addParam("rarityDataFile", "File that contains all the data related to the particular rarity round").setAction(
  async (taskArgs: TaskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    const { dataArgs } = require(`../data/airdrops/rarityfarming/${filename}.ts`)
    ...
    for (const [i, txGroup] of txData.entries()) {
      ...
      if (i === 0) {
        ...
        console.log(`Sending ${formatEther(totalAmount)} GHST to ${tokenIds.length} Gotchis (from ${tokenIds[0]} to ${tokenIds[tokenIds.length-1]})` )
      
    
        // const escrowFacet = await ethers.getContractAt("EscrowFacet",diamondAddress,signer)
      const escrowFacet=(await hre.ethers.getContractAt('EscrowFacet',diamondAddress)).connect(signer) as EscrowFacet
         const tx = await escrowFacet.batchDepositGHST(tokenIds,amounts, {gasPrice: parseUnits('5', 'gwei')})
